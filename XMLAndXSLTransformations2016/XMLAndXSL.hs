...
getAttribute k (Element _ atts _)
  | null res  = ""
  | otherwise = head res
  where
    res = [v | (k', v) <- atts, k == k']
...
getChildren n xml@(Element n' _ xmls)
  | n == n'   = [xml]
  | otherwise = concatMap (getChildren n) xmls
getChildren _ _
  = []
...
addChild xml (Element n atts xmls)
  = Element n atts (xmls ++ [xml])
...
parseAttributes s@(c : cs)
  | c == '>'  = ([], cs)
  | otherwise = ((name, value) : atts, rem'')
  where
    (name, rem)   = parseName (skipSpace s)
    skipEqQuotes  = tail (skipSpace (tail (skipSpace rem)))
    (value, rem') = break (=='\"') skipEqQuotes
    skipQuotes    = skipSpace (tail rem')
    (atts, rem'') = parseAttributes skipQuotes
...
parse' s@(c : c' : cs) (e : es)
  | c == '<' && c' == '/' = parse' (tail (dropWhile (/='>') cs)) (addChild e (head es) : tail es)
  | c == '<'              = parse' rem' ((Element name atts []) : e : es)
  | otherwise             = parse' rem'' (addChild (Text text) e : es)
  where
    (name, rem)   = parseName (c' : cs)
    (atts, rem')  = parseAttributes rem
    (text, rem'') = break (=='<') s
parse' _ ((Element _ _ (xml : _)) : _)
  = xml
